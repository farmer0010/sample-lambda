...
def setup_mock_table(mock_boto3_resource):
    mock_table = MagicMock()
...
@patch("backend.adapters.outbound.dynamodb_repository.boto3.resource")
def test_get_memos_limit_and_sort(mock_boto3_resource):
    setup_mock_table(mock_boto3_resource)
    repo = DynamoDBRepository(table_name="test-table")
...
@patch("backend.adapters.outbound.dynamodb_repository.boto3.resource")
def test_get_memos_filter_by_category(mock_boto3_resource):
    setup_mock_table(mock_boto3_resource)
    repo = DynamoDBRepository(table_name="test-table")
...
@patch("backend.adapters.outbound.dynamodb_repository.boto3.resource")
def test_get_memos_filter_search_keword(mock_boto3_resource):
    setup_mock_table(mock_boto3_resource)
    repo = DynamoDBRepository(table_name="test-table")
...
@patch("backend.adapters.outbound.dynamodb_repository.boto3.resource")
def test_get_memos_empty_search(mock_boto3_resource):
    setup_mock_table(mock_boto3_resource)
    repo = DynamoDBRepository(table_name="test-table")
