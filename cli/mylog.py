...
def add_memo(content: str, category: str):
    ...
    try:
        ...
        print(f"> [{category}] 메모가 클라우드에 저장되었습니다. (ID: {result.get('id')})")
...
def list_memos(category: str, limit: int, search: str):
    ...
    try:
        ...
        print(f"> 최근 글라우드메모 ({len(memos)}개 출력)")
        for memo in memos:
            date_str = memo.get('created_at', '')[2:10].replace('-', '.')
            print(f"  [ID: {memo['id']}] [{memo['category']}] {date_str} | {memo['content']}")

    except requests.exceptions.RequestException as e:
        print(f"메모 조회에 실패했습니다. \n에러: {e}")

def main():
    parser = argparse.ArgumentParser(description="개발자 전용 CLI 메모장 MyLog (AWS 연동)")
    ...
    parser_add.add_argument("-c", "--category", type=str, default="basic", help="카테고리 지정 (기본값: basic)")

    # 명령어 2. list
    parser_list = subparsers.add_parser("list", help="저장된 메모를 조회합니다.")
    parser_list.add_argument("category", type=str, nargs="?", default=None, help="특정 카테고리만 조회")
    parser_list.add_argument("-l", "--limit", type=int, default=5, help="출력할 메모 개수")
    parser_list.add_argument("-s", "--search", type=str, default=None, help="검색어 포함 내용 찾기")
